...
/**
 * Build a generic DIN 24296 header block for Excel workbooks with KSB branding.
...
function buildSummaryAoA(analyzedData) {
    const companyInfo = window.COMPANY_INFO || { name: 'KSB Pump', fullName: 'KSB Pump Solutions' };
    const appInfo = window.APP_INFO || { name: 'Interchangeable Parts List Creator', version: '2.0.0' };
    
    return [
        [companyInfo.fullName || 'KSB Pump Solutions'],
        ['INTERCHANGEABLE PARTS LIST — DIN 24296 TABLE 32 COMPLIANT'],
        [''],
        ['DOCUMENT INFORMATION'],
        ['Generated:', new Date().toLocaleString()],
        ['Generated By:', companyInfo.name || 'KSB Pump'],
...
function exportToExcel(analyzedData) {
    ...
    try {
        ...
        const filename = `KSB_Interchangeable_Parts_DIN24296_${analyzedData.totalPumps}pumps_${formatDateISO()}.xlsx`;
...
function exportToCSV(analyzedData) {
    ...
    try {
        ...
        downloadTextFile(
            csv,
            `KSB_Interchangeable_Parts_DIN24296_${analyzedData.totalPumps}pumps_${formatDateISO()}.csv`,
            'text/csv'
        );
...
function exportCommonSparesReport(analyzedData) {
    ...
    try {
        ...
        const filename = `KSB_Common_Spares_DIN24296_${analyzedData.totalPumps}pumps_${formatDateISO()}.xlsx`;
        XLSX.writeFile(wb, filename);
...
function exportComparisonMatrix(analyzedData) {
    ...
    try {
        ...
        const filename = `KSB_Pump_Comparison_DIN24296_${analyzedData.totalPumps}pumps_${formatDateISO()}.xlsx`;
        XLSX.writeFile(wb, filename);
...
function exportPumpInventorySummary(analyzedData) {
    ...
    try {
        ...
        const summaryAoA = [
            ...
            [''],
            ['PUMP DETAILS'],
            ['#', 'Pump Label', 'Model', 'Serial Number', 'Quantity', 'Location', 'Source File'],
        ];
        ...
        const filename = `KSB_Pump_Inventory_Summary_${totalPumps}pumps_${formatDateISO()}.xlsx`;
        XLSX.writeFile(wb, filename);
